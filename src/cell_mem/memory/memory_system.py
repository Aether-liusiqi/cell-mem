    ...
    def recall(
        ...
        try:
            ...
            if vector_ok and q_embedding is not None:
                ...
                # If vector search returns empty, fall back to FTS5 regardless
                # of pending count. Vector store may be empty (no embeddings yet,
                # or all processed but vector index empty), and FTS5 is the only
                # retrieval path left.
                if not results and query:
                    ...
                    if fts5_results:
                        ...
                        logger.debug("Vector search empty, fell back to FTS5 (%d results)",
                                   len(fts5_results))
            else:
